...
async load_profiles_and_regions() {
    ...
    update_aws_account_display() {
        ...
        if (this.is_connected && this.aws_account_id && accountContainer && accountId) {
            // Show account ID
            accountId.textContent = this.aws_account_id;
            // Show the container
        ...
        return `
            <div class="d-flex justify-content-between mt-3 gap-2">
                ${this.instances.find(i => i.id === instanceId).has_ssm ? `
                    <button class="btn btn-sm btn-dark" onclick="app.start_ssh('${instanceId}')">
                        <i class="bi bi-terminal"></i> SSH
                    </button>
                    <button class="btn btn-sm btn-primary" onclick="app.start_rdp('${instanceId}')">
                        <i class="bi bi-display"></i> RDP
                    </button>
                    <button class="btn btn-sm btn-purple text-white" onclick="app.show_custom_port_modal('${instanceId}')">
                        <i class="bi bi-arrow-left-right"></i> Port
                    </button>
                ` : ''}
                <button class="btn btn-sm btn-ottanio text-white" onclick="app.show_instance_details('${instanceId}')">
...
function setupInstanceFilter() {
    ...
    filterInput.addEventListener('input', app.debounce(() => {
        ...
        const filtered = !regex
            ? app.instances
            : app.instances.filter(inst => regex.test(inst.name));
